...
Napi::Array GetWordBoundaryPositions(const Napi::CallbackInfo& info) {
    ...
    icu_73::Locale icuLocale(language, country);


    // // create the BreakIterator instance
    UErrorCode err = U_ZERO_ERROR;
    icu_73::BreakIterator *iterator = icu_73::BreakIterator::createWordInstance(icuLocale, err);
    if (U_FAILURE(err)) {
        icu_73::ErrorCode errCode;
        errCode.set(err);
    ...
    icu_73::UnicodeString uTextValue(textValue.ToString().Utf8Value().c_str(), "UTF-8");
    ...
    while (currentBoundary != icu_73::BreakIterator::DONE) {
